...
pub(crate) fn emit_decaf_diagnostic(
    ...
    let diagnostic = match diagnostic {
        checker::Diagnostic::Global { reason, kind } => Diagnostic {
            severity: decaf_diagnostic_to_severity(kind),
            code: None,
            message: reason,
            labels: Vec::new(),
            notes: Vec::default(),
        ...
        checker::Diagnostic::PositionWithAdditionLabels {
            reason,
            position,
            labels,
            kind,
            ...
            let (labels, notes) = labels
                .into_iter()
                .partition::<Vec<_>, _>(|(_, value)| value.is_some());
...
fn decaf_diagnostic_to_severity(kind: checker::DiagnosticKind) -> Severity {
    match kind {
