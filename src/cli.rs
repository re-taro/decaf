...
use crate::{error_handling::emit_decaf_diagnostic, temp::Output, utilities};
...
pub fn run_cli() {
    ...
    let command = match FromArgs::from_args(&["decaf-cli"], arguments.as_slice()) {
...
fn build(build_arguments: BuildArguments) -> Result<(), ()> {
    ...
    // let plugins = vec![
    // 	// decaf_web_framework::get_plugin(),
    // 	test_plugin,
    // ];
    ...
    match result {
        Ok(BuildOutput {
            ...
            for error in temp_warnings_and_infos {
                emit_decaf_diagnostic(&fs, error).unwrap();
            }
            // print_error_warning_info_handler(error_warning_info_handler);
