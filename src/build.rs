...
pub struct BuildOutput {
    ...
    pub check_output: checker::CheckOutput<checker::synthesis::DecafParser>,
}

pub struct FailedBuildOutput(pub checker::CheckOutput<checker::synthesis::DecafParser>);
...
pub struct BuildConfig {
    ...
    pub other_transformers: Option<DecafParsePostCheckVisitors>,
}
...
pub type DecafParsePostCheckVisitors =
    parser::visiting::VisitorsMut<CheckingOutputWithoutDiagnostics>;

pub type OwnedDecafModule =
    <checker::synthesis::DecafParser as checker::ASTImplementation>::OwnedModule;
pub type SynthesisedDecafModule = checker::features::modules::SynthesisedModule<OwnedDecafModule>;
...
pub struct CheckingOutputWithoutDiagnostics {
    ...
    pub modules: HashMap<SourceId, SynthesisedDecafModule>,
}
...
pub fn build<T: crate::ReadFromFS>(
    ...
    if !result.diagnostics.contains_error() {
        ...
        if config.tree_shake {
            transformers.expression_visitors_mut.push(Box::new(
                crate::transformers::optimisations::ExpressionOptimiser,
            ));

            transformers.statement_visitors_mut.push(Box::new(
