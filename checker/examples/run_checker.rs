...
fn main() {
    use decaf_checker::{check_project, synthesis, Diagnostic, TypeCheckOptions};
    ...
    let overrides_dts_path = Path::new("checker")
        .join("definitions")
        .join("overrides.d.ts");
    ...
    let type_definition_files = if no_lib {
        ...
        let definition_file = if use_simple {
            ...
            decaf_checker::INTERNAL_DEFINITION_FILE_PATH.into()
        };
        vec![definition_file]
    };
    ...
    let result = check_project::<_, synthesis::DecafParser>(
        entry_points,
    ...
    if args.iter().any(|arg| arg == "--time") {
        ...
        for diagnostic in result.diagnostics {
            let prefix: &str = match diagnostic.kind() {
                decaf_checker::DiagnosticKind::Error => "ERROR",
                decaf_checker::DiagnosticKind::Warning => "WARNING",
                decaf_checker::DiagnosticKind::Info => "INFO",
            };
            match diagnostic {
...
#[cfg(not(feature = "decaf-parser"))]
fn main() {
    println!("Requires 'decaf-parser'")
}
