...
use decaf_checker::{generate_cache, synthesis::DecafParser};

fn main() {
    let input = env::args()
        .nth(1)
        .expect("expected path to definition file");
    ...
    let cache = generate_cache::<_, DecafParser>(PathBuf::from(input).as_path(), &reader, ());
