    ...
    pub(crate) fn get_file(
        ...
        if let Some(source) = get_source_at_path {
            ...
            if let Ok(content) = String::from_utf8(content) {
                ...
                Some(File::Source(
                    source_id,
                    self.files.get_file_content(source_id),
                ))
            } else {
    ...
    A: crate::ASTImplementation,
{
    ...
    pub fn raise_decidable_result_error(&mut self, span: SpanWithSource, value: bool) {
        self.diagnostics_container
            .add_error(TypeCheckWarning::DeadBranch {
...
impl<A: crate::ASTImplementation> CheckOutput<A> {
    ...
    pub fn get_type_at_position_with_span(
        ...
        if let Some(module) = self.modules.get(&source) {
            module
                .get_instance_at_position_with_span(pos)
                .map(|(instance, range)| {
                    (
                        crate::types::printing::print_type(
                            instance.get_value_on_ref(),
...
pub fn check_project<T: crate::ReadFromFS, A: crate::ASTImplementation>(
    ...
    for point in &entry_points {
        ...
        let current = checking_data
            .options
            .measure_time
            .then(std::time::Instant::now);

        let entry_content =
            if let Some(source) = checking_data.modules.files.get_source_at_path(point) {
                ...
                let source = checking_data
                    .modules
                    .files
                    .new_source_id(point.clone(), content.clone());
                Some((source, content))
            } else {
                None
            };
        ...
        if let Some((source, content)) = entry_content {
            ...
            checking_data
                ...
                .add_error(TypeCheckError::CannotOpenFile {
...
const CACHE_MARKER: &[u8] = b"decaf-cache-file";
...
pub(crate) fn add_definition_files_to_root<T: crate::ReadFromFS, A: crate::ASTImplementation>(
    ...
    for path in type_definition_files {
        let chronometer = checking_data
            .options
            .measure_time
            .then_some(&mut checking_data.chronometer);
...
fn deserialize_cache<T: ReadFromFS, A: ASTImplementation>(
    ...
    let source_id = {
        ...
        let get_source_at_path = checking_data
            .modules
            .files
            .get_source_at_path(Path::new(&path));
        ...
        if let Some(source_id) = get_source_at_path {
            ...
            checking_data
                .modules
                .files
                .new_source_id(path.into(), source_content)
        }
    };
...
pub fn get_closest<'a, 'b>(
    ...
    let candidates = items
        .filter(|item| levenshtein(closest_one, item) <= MIN_DISTANCE)
        .collect::<Vec<&str>>();
