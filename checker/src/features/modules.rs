...
impl Exported {
    ...
    pub fn get_export(
        ...
        let variable = if type_only {
            ...
            self.named
                .get(want)
                .map(|(name, mutability)| (*name, *mutability))
        };
                    ...
                    if let Some((variable, mutability)) = exported_variable {
                        ...
                        if also_export {
                            ...
                            } = environment.context_type.scope
                            {
                                exported
                                    .named
                                    .insert(part.r#as.to_owned(), (variable, mutability));
                            }
                        }
                    }
            ...
            if let Ok(Ok(ref exports)) = exports {
                for (name, (variable, mutability)) in exports.named.iter() {
                    ...
                    } = environment.context_type.scope
                    {
                        exported
                            .named
                            .insert(name.clone(), (*variable, *mutability));
                    }
                }
            } else {
                // TODO ??
...
pub fn import_file<T: crate::ReadFromFS, A: crate::ASTImplementation>(
    ...
    fn get_module<'a, T: crate::ReadFromFS, A: crate::ASTImplementation>(
        ...
        let existing = checking_data
            .modules
            .files
            .get_source_at_path(full_importer);
        if let Some(existing) = existing {
    ...
    fn get_package_from_node_modules<T: crate::ReadFromFS>(
        ...
        // TODO error
        let package_json = fs_reader
            .read_file(&PathBuf::from(&package_json_path))
            .ok_or(())?;
        ...
        file_path.ok_or(()).map(|entry| {
            (
                ...
                definition_file_path.map(|dfp| package_root.join(dfp)),
            )
        })
    }

    let result = if to_import.starts_with('.') {
        let from_path = checking_data
            .modules
            .files
            .get_file_path(environment.get_source());
