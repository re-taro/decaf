//! Contains implementations of specific JavaScript items and how Decaf handles them.
//! Contains
...
pub fn type_of_operator(on: TypeId, types: &mut TypeStore) -> TypeId {
    if let Some(constraint) = get_constraint(on, types) {
        if let Some(str_name) = type_to_js_string_name(constraint) {
            ...
            types.register_type(crate::Type::Constructor(
...
pub fn delete_operator(
...

    {
        let constraint = environment
            .get_object_constraint(rhs)
            .or_else(|| get_constraint(rhs, types));
    ...
    environment
        .info
        .delete_property(rhs, (publicity, under.into_owned()), position, dependency);
...
pub fn in_operator(
    ...
    if get_constraint(rhs, types).is_some() {
        ...
        environment
            .info
            .events
            .push(crate::events::Event::Miscellaneous(
...
pub mod tsc {
    ...
    pub fn as_cast(on: TypeId, cast_to: TypeId, types: &mut TypeStore) -> Result<TypeId, ()> {
        ...
        if can_cast {
            ...
            let cast_to = if cast_to == TypeId::ANY_TYPE {
                TypeId::ERROR_TYPE
            } else {
                cast_to
            };

            Ok(types.register_type(Type::Narrowed {
                narrowed_to: cast_to,
                from: on,
            }))
        } else {
    ...
    pub fn check_satisfies<T: crate::ReadFromFS, A: crate::ASTImplementation>(
        ...
        if result.is_mismatch() {
            ...
            checking_data.diagnostics_container.add_error(
                diagnostics::TypeCheckError::NotSatisfied {
                    at,
                    expected,
                    found,
                },
