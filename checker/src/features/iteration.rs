    ...
    match behavior {
        IterationBehavior::While(condition) => {
            ...
            run_iteration_block(
                IterationKind::Condition {
                    under: fixed_iterations.ok(),
                    postfix_condition: false,
                },
                &events,
                &application_input,
        ...
        IterationBehavior::ForIn { lhs, rhs } => {
            ...
            let variable =
                checking_data
                    .types
                    .register_type(Type::RootPolyType(PolyNature::Parameter {
            ...
            if let Some(result) = result {
                ...
                match result {
                    ...
                    ApplicationResult::Continue { carry, position } => {
                        return Some(ApplicationResult::Continue {
                            carry: carry - 1,
                            position,
                        });
                    }
                    ApplicationResult::Break { carry, position } => {
                        return Some(ApplicationResult::Break {
                            carry: carry - 1,
                            position,
                        });
                    }
...
fn evaluate_unknown_iteration_for_loop(
    ...
    // add event
    {
        ...
        get_latest_info
            .events
            .push(Event::EndOfControlFlow(events.len() as u32));
    }
}
    ...
    }) = condition_ty
    {
        ...
        }) = reference_ty
        {
            if let RootReference::Variable(possible_changing_variable_id) = less_than_reference {
                ...
                {
                    let changed = if let Some((_start, _end)) = loop_variables
                        .as_ref()
                        .and_then(|vs| vs.get(roof_id).copied())
                    {
                ...
                }) = value_after_running_expressions_in_loop
                {
                    ...
                    return Ok(LoopStructure {
                        start,
                        roof,
                        increment_by: *increments_by,
                    });
                }
            } else {
