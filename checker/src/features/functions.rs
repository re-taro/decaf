...
pub fn register_expression_function<T: crate::ReadFromFS, A: crate::ASTImplementation>(
    ...
    let name = if let Some(name) = name {
        checking_data
            .types
            .new_constant_type(Constant::String(name))
    } else {
...
pub fn synthesise_function_default_value<'a, T: crate::ReadFromFS, A: ASTImplementation>(
    ...
    let is_undefined_condition =
        checking_data
            .types
            .register_type(Type::Constructor(Constructor::TypeExtends(
                types::TypeExtends {
                    item: parameter_ty,
    ...
    let result =
        checking_data
            .types
            .register_type(Type::Constructor(Constructor::ConditionalResult {
    ...
    F: SynthesisableFunction<A>,
{
    ...
    let kind: FunctionKind<A> = match behavior {
        ...
        FunctionRegisterBehavior::ArrowFunction {
            ...
            if let Some((or, _)) = expected_parameters
                .as_ref()
                .and_then(|a| a.get_parameter_type_at_index(0))
            {
        ...
        FunctionRegisterBehavior::ObjectMethod {
            is_async,
            is_generator,
            expecting,
            name,
    ...
    if function.has_body() {
        ...
        let this_constraint = function
            .this_constraint(&mut function_environment, checking_data)
            .or(this_shape);
        ...
        if let Scope::Function(ref mut scope) = function_environment.context_type.scope {
            match scope {
                ...
                | FunctionScope::MethodFunction {
                    ...
                    if let FunctionBehavior::Method {
                        ref mut free_this_id,
                        ..
                    } = behavior
                    {
                ...
                FunctionScope::Function {
                    ...
                    // TODO ahhhh
                    if let FunctionBehavior::Function {
                        this_id: ref mut free_this_id,
                        ..
                    } = behavior
                    {
                ...
                FunctionScope::Constructor {
                    ...
                    if let Some((prototype, properties)) = constructor {
                        ...
                        *this_object_type = this_constructed_object;
                        ...
                        if let FunctionBehavior::Constructor {
                            ref mut this_object_type,
                            ..
                        } = behavior
                        {
        ...
        if let Some(closed_over_variables) = base_environment
            .context_type
    ...
    if let Type::FunctionReference(func_id) = ty {
        ...
        let type_arguments = match structure_generic_arguments {
            types::generics::generic_type_arguments::GenericArguments::ExplicitRestrictions(
                ...
                parent: None,
                arguments: explicit_restrictions
                    .into_iter()
                    .map(|(l, (r, _))| (l, r))
                    .collect(),
