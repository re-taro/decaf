    ...
    match id {
        ...
        | "floor" | "ceil" | "trunc" | "sqrt" | "cbrt" | "abs" => {
            ...
            Ok(ConstantOutput::Value(
                types.new_constant_type(Constant::Number(num)),
            ))
        }
        "imul" => {
        ...
        "print_constraint" => {
            ...
            if let Some(constraint) = constraint {
                ...
                Ok(ConstantOutput::Diagnostic(format!(
                    "Constraint is: {constraint_as_string}"
                )))
            } else {
                Ok(ConstantOutput::Diagnostic(
                    "No associate constraint".to_owned(),
                ))
            }
        }
            ...
            | Type::FunctionReference(func) = get_type_by_id
            {
                ...
                if id.ends_with("rust") {
                    ...
                    match effects {
                        ...
                        FunctionEffect::Constant {
                            identifier,
                            may_throw: _,
                        } => {
                            format!("Constant: {identifier}")
                        }
                        FunctionEffect::InputOutput {
                            identifier,
                            may_throw: _,
                        } => {
        ...
        "freeze" => {
            if let Some(on) = (arguments.len() == 1)
                .then(|| arguments[0].non_spread_type().ok())
                .flatten()
            {
                environment
                    .info
                    .frozen
                    .insert(on, ObjectProtectionState::Frozen);
        ...
        "seal" => {
                ...
                .flatten()
            {
                environment
                    .info
                    .frozen
                    .insert(on, ObjectProtectionState::Sealed);
                Ok(ConstantOutput::Value(on))
            } else {
                Err(ConstantFunctionError::CannotComputeConstant)
            }
        ...
        "preventExtensions" => {
                ...
                .flatten()
            {
                environment
                    .info
                    .frozen
                    .insert(on, ObjectProtectionState::NoExtensions);
                Ok(ConstantOutput::Value(on))
            } else {
                Err(ConstantFunctionError::CannotComputeConstant)
            }
        ...
        "defineProperty" => {
            ...
            if let [on, property, descriptor] = arguments {
                ...
                environment
                    .info
                    .register_property(on, Publicity::Public, under, value, call_site);
        ...
        "getOwnPropertyDescriptor" => {
            if let [on, property] = arguments {
                ...
                let value = crate::types::properties::resolver(
                    ...
                    (
                        Publicity::Public,
                        &PropertyKey::from_type(property, types),
                        None,
                    ),
                    environment,
                    types,
        ...
        "regexp:constructor" => {
            let pattern = types.get_type_by_id(
                arguments
                    .first()
                    .unwrap()
                    .non_spread_type()
            ...
            let flags = arguments
                .get(1)
                .map(|a| types.get_type_by_id(a.non_spread_type().expect("flags")));
        ...
        // 	}
        // }
        "debug_state" => Ok(ConstantOutput::Diagnostic(format!(
            "State={:?}",
            environment.info.state
        ))),
        "debug_context" => Ok(ConstantOutput::Diagnostic(environment.debug())),
        "context_id" => Ok(ConstantOutput::Diagnostic(format!(
            "in {:?}",
            environment.context_id
        ))),
