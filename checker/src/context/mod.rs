...
impl<T: ContextType> Context<T> {
    ...
    pub(crate) fn debug(&self) {
        ...
        for (indent, parent) in enumerate {
            ...
            println!(
                "{}Context#{} - {}",
                indent,
                get_ctx!(parent.context_id.clone()).0,
                ty
            );
    ...
    pub fn get_type_from_name(&self, name: &str) -> Option<TypeId> {
        self.parents_iter()
            .find_map(|env| get_ctx!(env.named_types.get(name)))
            .cloned()
    }
    ...
    pub(crate) fn get_variable_name(&self, id: &VariableId) -> String {
        self.parents_iter()
            .find_map(|env| get_ctx!(env.variable_names.get(id)))
            .cloned()
            .unwrap()
    }
    ...
    pub fn is_frozen(&self, value: TypeId) -> Option<TypeId> {
        self.parents_iter()
            .find_map(|env| get_ctx!(env.frozen.get(&value)))
            .cloned()
    }
