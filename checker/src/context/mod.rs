...
impl<T: ContextType> Context<T> {
    ...
    pub(crate) fn debug(&self) {
        ...
        for (indent, parent) in enumerate {
            ...
            println!(
                "{}Context#{} - {}",
                indent,
                get_env!(parent.context_id.clone()).0,
                ty
            );
        ...
        match types.get_type_by_id(on) {
            ...
            Type::Constructor(constructor) => match constructor.clone() {
                ...
                Constructor::Property { on, under } => {
                    ...
                    // Bad
                    let is_open_poly = on_constraint
                        .as_ref()
                    ...
                    let on_base = on_constraint.unwrap_or(PolyBase::Fixed {
                        to: on,
                        is_open_poly: false,
                    });

                    // TODO this should also create mutable-ness
                    let property_base = property_constraint.unwrap_or(PolyBase::Fixed {
                        to: under,
                        is_open_poly: false,
                    });
                ...
                Constructor::TypeRelationOperator(op) => match op {
                    crate::types::TypeRelationOperator::Extends { .. } => Some(PolyBase::Fixed {
                        to: TypeId::BOOLEAN_TYPE,
                        is_open_poly: false,
                    }),
                },
            },
        ...
        match types.get_type_by_id(on) {
            ...
            Type::Or(left, right) => {
                ...
                let left = self
                    ...
                    .map(Box::new);
                let right = self
                    .get_fact_about_type(*right, resolver, data, types)
                    .map(Box::new);
    ...
    pub fn get_type_from_name(&self, name: &str) -> Option<TypeId> {
        self.parents_iter()
            .find_map(|env| get_env!(env.named_types.get(name)))
            .cloned()
    }
    ...
    pub(crate) fn get_variable_name(&self, id: &VariableId) -> String {
        self.parents_iter()
            .find_map(|env| get_env!(env.variable_names.get(id)))
            .cloned()
            .unwrap()
    }
    ...
    pub fn is_frozen(&self, value: TypeId) -> Option<TypeId> {
        self.parents_iter()
            .find_map(|env| get_env!(env.frozen.get(&value)))
            .cloned()
    }
    ...
    pub fn new_lexical_environment_fold_into_parent<U: crate::FSResolver, Res>(
        ...
        Res,
        Option<(Vec<Event>, HashMap<Reference, TypeId>)>,
        ContextId,
    ) {
    ...
    pub(crate) fn get_current_constructor(&self) -> Option<TypeId> {
        self.parents_iter().find_map(|env| {
            if let GeneralEnvironment::Syntax(Context {
                context_type:
                    Syntax {
                        scope: Scope::Function { constructor_on, .. },
                        ..
                    },
                ..
            }) = env
    ...
    pub fn attach_to_existing_function(
        ...
        self.variable_current_value
            .insert(VariableId(variable_id), r#type);
    }
