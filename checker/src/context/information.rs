...
impl LocalInformation {
    ...
    pub(crate) fn extend(&mut self, other: LocalInformation, condition: Option<TypeId>) {
        ...
        self.variable_current_value
            .extend(other.variable_current_value);
        ...
        self.closure_current_values
            .extend(other.closure_current_values);
    ...
    pub(crate) fn extend_ref(&mut self, other: &LocalInformation) {
        ...
        self.queued_events
            .extend(other.queued_events.iter().cloned());
        self.variable_current_value
            .extend(other.variable_current_value.iter().clone());
        self.prototypes.extend(other.prototypes.iter().clone());
        self.current_properties.extend(
            other
                .current_properties
                .iter()
                .map(|(l, r)| (*l, r.clone())),
        );
        self.closure_current_values.extend(
            other
        ...
        self.narrowed_values
            .extend(other.narrowed_values.iter().copied());
...
pub trait InformationChain {
    ...
    fn get_narrowed(&self, for_ty: TypeId) -> Option<TypeId> {
        self.get_chain_of_info()
            .find_map(|info| info.narrowed_values.get(&for_ty).copied())
    }
...
pub fn merge_info(
    ...
    let (new_state, carry_information_from) = match (
        ...
        otherwise
            .as_ref()
            .map_or(ReturnState::Continued, |o| o.state.clone()),
    ) {
        ...
        (ReturnState::Continued, ReturnState::Finished(returned)) => (
            ReturnState::Rolling {
                under: types.new_logical_negation_type(condition),
                returned,
            },
            Some(true),
        ),
        ...
        (ReturnState::Rolling { under, returned }, ReturnState::Continued) => (
            ReturnState::Rolling {
                under: types.new_logical_and_type(condition, under),
                returned,
            },
            None,
        ),
        (
            ReturnState::Rolling {
                under: truthy_under,
                returned: truthy_returned,
            },
            ReturnState::Rolling {
                under: otherwise_under,
                returned: otherwise_returned,
            },
        ) => {
    ...
    if truthy_events + otherwise_events != 0 {
        ...
        onto.events
            .push(Event::EndOfControlFlow(truthy_events + otherwise_events));
    }
