...
pub(crate) fn get_type_from_reference<'a, S: ContextType, T: crate::FSResolver>(
    ...
    match reference {
        TypeReference::StringLiteral(value, _) => Ok(checking_data
            .types
        ...
        TypeReference::BooleanLiteral(value, _) => Ok(checking_data
            .types
        ...
        TypeReference::NameWithGenericArguments(name, arguments, position) => {
                ...
                .get_parameters()
            {
                ...
                let with = parameters
                    ...
                    .collect::<Vec<_>>()
                    .into_iter()
                    .zip(arguments.iter().map(|type_reference| {
                        environment.get_type_handle_errors(type_reference, checking_data)
                    }));
        ...
        TypeReference::ObjectLiteral(members, type_id, _) => {
            let ty_id = checking_data.types.new_type(Type::Object(
