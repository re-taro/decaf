...
pub(super) fn synthesize_expression<T: crate::FSResolver>(
    ...
    let instance: Instance = match expression {
        ...
        Expression::RegexLiteral {
            ...
            return checking_data
                .types
                .new_constant_type(Constant::Regexp(pattern.clone()));
        }
        ...
        Expression::ArrayLiteral(elements, _) => {
            ...
            let len = checking_data.types.new_constant_type(Constant::Number(
        ...
        Expression::TemplateLiteral(TemplateLiteral {
            ...
            let tag = tag
                .as_ref()
                .map(|expr| synthesize_expression(expr, environment, checking_data));
        ...
        Expression::SuperExpression(reference, position) => match reference {
            SuperReference::Call { arguments } => {
                let constructor = environment
                    .get_current_constructor()
                    .expect("not in constructor");
        ...
        | Expression::PrefixComment(_comment, expression, _) => Instance::RValue(
        ...
        Expression::IsExpression(is_expr) => Instance::RValue(synthesize_is_expression(
            is_expr,
            environment,
            checking_data,
        )),
