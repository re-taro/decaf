...
pub fn synthesise_type_annotation<T: crate::ReadFromFS>(
    ...
    let ty = match annotation {
        ...
        TypeAnnotation::StringLiteral(value, ..) => checking_data
            .types
            .new_constant_type(Constant::String(value.clone())),
        TypeAnnotation::NumberLiteral(value, _) => {
            let constant = Constant::Number(
                f64::try_from(value.clone())
                    .expect("big int number type")
                    .try_into()
                    .unwrap(),
            );
            checking_data.types.new_constant_type(constant)
        }
        TypeAnnotation::BooleanLiteral(value, _) => checking_data
            .types
            .new_constant_type(Constant::Boolean(*value)),
        ...
        TypeAnnotation::ObjectLiteral(members, _) => {
            ...
            checking_data
                .types
                .new_anonymous_interface_type(properties.0)
        }
        ...
        TypeAnnotation::Index(being_indexed, indexer, _) => {
            ...
            checking_data
                .types
                .new_property_on_type_annotation(being_indexed, indexer, environment)
        }
        ...
        TypeAnnotation::Conditional {
            ...
            let result_union = checking_data
                .types
                .new_or_type(truthy_result, otherwise_result);
        ...
        TypeAnnotation::TemplateLiteral { parts, last, .. } => {
            ...
            for (static_part, dynamic_part) in parts {
                let lhs = checking_data
                    .types
                    .new_constant_type(Constant::String(static_part.to_owned()));
                ...
                let rhs = if let TypeAnnotation::Infer {
                    name,
                    extends: None,
                    position: _,
                } = dynamic_part
                ...
                acc = checking_data
                    .types
                    .register_type(Type::Constructor(constructor));
            }
            if last.is_empty() {
                acc
            } else {
                let lhs = checking_data
                    .types
                    .new_constant_type(Constant::String(last.to_owned()));
        ...
        TypeAnnotation::Infer {
            ...
            if let Scope::TypeAnnotationCondition {
                ref mut infer_parameters,
        ...
        TypeAnnotation::Symbol {
            ...
            if let Some(name) = name {
                ...
                if name == "iterator" {
                    ...
                    checking_data
                        .types
                        .new_constant_type(Constant::Symbol { key: name.clone() })
                }
            } else {
                TypeId::SYMBOL_TYPE
            }
    ...
    if checking_data.options.store_type_mappings {
        checking_data.local_type_mappings.types_to_types.push(
            annotation
                .get_position()
                .with_source(environment.get_source()),
            ty,
...
pub(crate) fn comment_as_type_annotation<T: crate::ReadFromFS>(
    ...
    let possible_declaration = possible_declaration
        .strip_prefix('*')
        .unwrap_or(possible_declaration);
