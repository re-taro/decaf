...
pub(crate) fn type_function_parameters_from_reference<T: crate::FSResolver>(
    ...
    let rest_parameter = reference_parameters
        ...
        .map(|rest_parameter| {
            let ty =
                environment.get_type_handle_errors(&rest_parameter.type_reference, checking_data);
...
fn hoist_function_parameters(
    ...
    for parameter in parameters.optional_parameters.iter() {
        match parameter {
            parser::OptionalOrWithDefaultValueParameter::Optional {
                name,
                type_reference,
            } => {
                todo!()
    ...
    let optional_parameters = ast_parameters
        ...
        .map(|parameter| match parameter {
            parser::OptionalOrWithDefaultValueParameter::Optional {
                ...
                let VariableIdentifier::Standard(name, variable_id, position) = name else {
                    panic!()
                };
...
fn synthesize_parameter_type_reference<T: crate::FSResolver>(
    ...
    if let Some(type_reference) = type_reference {
        ...
        if !matches!(
            ...
            checking_data
                .types
                .new_type(Type::RootPolyType(PolyNature::Parameter {
    ...
    BasedOnWhat::Body: SynthesizableFunctionBody,
{
    ...
    let (this_extends_something, constructor_on) = if let FunctionNature::ClassConstructor {
        class_prototype,
        class_constructor,
    } = nature
    {
        ...
        .new_lexical_environment_fold_into_parent(
            ...
            |environment, checking_data| {
                ...
                let return_type =
                    function
                        .body
                        .synthesize_function_body(environment, checking_data, chain);
