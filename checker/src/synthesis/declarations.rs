    ...
    match declaration {
        ...
        Declaration::Class(class) => {
            ...
            if let Some(variable) = class.on.name.get_variable_id(environment.get_source()) {
                environment
                    ...
                    .insert(variable, constructor);
            }
        }
        Declaration::Export(exported) => match &exported.on {
            parser::declarations::ExportDeclaration::Item {
                exported,
                position: _,
            } => {
                match exported {
        ...
        Declaration::Enum(r#enum) => {
            ...
            environment
                .info
                .variable_current_value
                .insert(variable, basis.build_object());
        }
                ...
                let exported = exported.then(|| {
                    let restriction = checking_data
                        ...
                        .get(&(
                            ...
                            variable_declaration.position.start,
                        ))
                        .map(|(first, _)| *first);
                    VariableMutability::Mutable {
                        reassignment_constraint: restriction,
                    }
                });
