...
>(
    ...
    if let Some(class_type) = existing_id {
        ...
        let class_type = checking_data.types.register_type(Type::Class {
            name: name.clone(),
...
>(
    ...
    let extends = class.extends.as_ref().map(|extends_expression| {
        let extends = synthesise_expression(
            extends_expression,
            environment,
            checking_data,
            TypeId::ANY_TYPE,
        ...
        crate::utilities::notify!(
            "{:?}",
            (extends, checking_data.types.get_type_by_id(extends))
        );
    ...
    if let Some(extends) = extends {
        ...
        let copied = environment
            .get_chain_of_info()
            .find_map(|info| info.prototypes.get(&extends))
            .copied();
        if let Some(extends_prototype) = copied {
            environment
                .info
                .prototypes
                .insert(class_prototype, extends_prototype);
        }
    }
        ...
        match &member.on {
            ClassMember::Method(false, method) => {
                ...
                } = checking_data.types.get_type_by_id(class_prototype)
                {
                    ...
                    checking_data
                        .types
                        .register_type(Type::PartiallyAppliedGenerics(
                            crate::types::PartiallyAppliedGenerics {
                                on: class_prototype,
                                arguments,
                            },
                        ))
            ...
            ClassMember::Property(false, property) => {
                ...
                // TODO restriction
                properties.push(ClassValue {
                    publicity,
                    key,
                    value: property.value.as_deref(),
                });
            }
    ...
    let name_as_type_id = if let Some(name) = class.name.as_option_str() {
        checking_data
            .types
            .new_constant_type(crate::Constant::String(name.to_owned()))
    } else {
    ...
    if let Some(variable) = class.name.get_variable_id(environment.get_source()) {
        environment
            ...
            .insert(variable, class_variable_type);
    }
...
pub(super) fn register_statement_class_with_members<T: crate::ReadFromFS>(
    ...
    if let Some(type_parameters) = type_parameters {
        ...
        {
            let crate::context::LocalInformation {
                current_properties,
                prototypes,
                ..
            } = sub_environment.info;
        ...
        match &member.on {
            ClassMember::Method(initial_is_static, method) => {
                ...
                if *initial_is_static {
                    ...
                    environment
                        .info
                        .register_property_on_type(class_type, publicity, under, value);
                }
            }
