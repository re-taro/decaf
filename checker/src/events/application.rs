        ...
        match &event {
            ...
            Event::SetsVariable(variable, value, position) => {
                if unknown_mode {
                    ...
                    let in_above_environment = top_environment
                        .variables
                        .values()
                        .any(|var| var.get_id() == *variable);
                    ...
                    if !in_above_environment {
                        info.events
                            .push(Event::SetsVariable(*variable, new_value, *position));
                    }
                ...
                match fixed_result {
                    Decidable::Known(result) => {
                        let (start, end) = if result {
                            ...
                            (
                                offset + truthy_events,
                                offset + truthy_events + otherwise_events,
                            )
                        };
            ...
            Event::Iterate {
                ...
                let kind = match kind {
                    IterationKind::Condition {
                        ...
                        under: under
                            .map(|under| under.specialise(type_arguments, top_environment, types)),
                        ...
                        *position,
                        ...
                        Err(err) => {
                            diagnostics.errors.push(match err {
                                CannotDeleteFromError::Constraint {
                                    constraint,
                                    position,
