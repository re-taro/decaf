...
impl FunctionType {
    ...
    pub(crate) fn call<'a, E: CallCheckingBehavior>(
        ...
        let mut type_arguments = TypeArguments {
            structure_arguments: parent_type_arguments.clone(),
            local_arguments,
        };
        ...
        let SeedingContext {
            ...
            let mut seeding_context = SeedingContext {
                type_arguments,
                locally_held_functions: HashMap::new(),
            };
            ...
            for (idx, parameter) in self.parameters.parameters.iter().enumerate() {
                ...
                if let Some((argument_type, argument_position)) = argument_type_and_pos {
                    ...
                    if E::CHECK_PARAMETERS {
                        ...
                        seeding_context
                            .type_arguments
                            .set_id(parameter.ty, *argument_type, &types);
                    }
                } else if let Some(value) = parameter.missing_value {
                    // TODO evaluate effects
                    seeding_context
                        .type_arguments
                        .set_id(parameter.ty, value, types)
                } else {
                    // TODO group
            ...
            if self.parameters.parameters.len() < arguments.len() {
                if let Some(ref rest_parameter) = self.parameters.rest_parameter {
                    for (idx, argument) in arguments
                        .iter()
                        .enumerate()
                        .skip(self.parameters.parameters.len())
                    {
