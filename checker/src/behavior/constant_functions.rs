...
pub(crate) fn call_constant_function(
    ...
    match id {
        ...
        | "trunc" | "sqrt" | "cbrt" | "abs" => {
            ...
            match try_into {
                Ok(try_into) => Ok(ConstantResult::Value(
                    types.new_constant_type(Constant::Number(try_into)),
                )),
        ...
        "is_dependent" => Ok(ConstantResult::Diagnostic(format!(
            "is dependent {:?}",
            types
                .get_type_by_id(arguments.first().unwrap().into_type().unwrap())
                .is_dependent()
        ))),
