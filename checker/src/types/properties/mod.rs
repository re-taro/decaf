...
impl crate::BinarySerializable for PropertyKey<'static> {
    ...
    fn deserialize<I: Iterator<Item = u8>>(iter: &mut I, source: source_map::SourceId) -> Self {
        match iter.next().unwrap() {
            0 => Self::String(Cow::Owned(crate::BinarySerializable::deserialize(
                iter, source,
            ))),
...
pub(crate) fn get_simple_property_value(
    ...
    let value = get_property_unbound(
        (on, None),
        (Publicity::Public, property, None),
        false,
        ctx,
        types,
    )
...
impl PropertyValue {
    ...
    pub fn is_optional_simple(&self) -> bool {
        if let PropertyValue::ConditionallyExists {
            condition,
            truthy: _,
        } = self
        {
    ...
    pub fn is_writable_simple(&self) -> bool {
        ...
        } = self
        {
            ...
            !matches!(
                descriptor.writable,
                TypeId::TRUE | TypeId::WRITABLE_KEY_ARGUMENT
            )
        } else {
            false
        }
