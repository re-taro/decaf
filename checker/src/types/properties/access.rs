...
pub(crate) fn resolver(
    ...
    info_chain
        .get_chain_of_info()
        .find_map(|info: &LocalInformation| {
...
pub(crate) fn get_property_unbound(
    ...
    fn get_property_on_type_unbound(
        ...
        match types.get_type_by_id(on) {
            ...
            Type::SpecialObject(SpecialObject::Proxy(proxy)) => Ok(
                LogicalOrValid::NeedsCalculation(NeedsCalculation::Proxy(*proxy, on)),
            ),
    ...
    if let PropertyKey::Type(key) = under {
            ...
            get_type_as_conditional(key, types)
        {
            let left = get_property_unbound(
                ...
                (
                    publicity,
                    &PropertyKey::Type(truthy_result),
                    under_type_arguments,
                ),
                require_both_logical,
                info_chain,
                types,
            ...
            let right = get_property_unbound(
                ...
                (
                    publicity,
                    &PropertyKey::Type(otherwise_result),
                    under_type_arguments,
                ),
                require_both_logical,
                info_chain,
