...
pub fn set_property<B: CallCheckingBehavior>(
    ...
    {
        let object_constraint = environment
            .get_object_constraint(on)
            .or_else(|| get_constraint(on, types));
    ...
    let current_property = get_property_unbound(
        (on, None),
        (publicity, under, None),
        false,
        environment,
        ...
        PropertyValue::Deleted | PropertyValue::Value(..) => {
            ...
            let value = if let Some(descriptor) = descriptor {
                PropertyValue::Configured {
                    on: Box::new(PropertyValue::Value(new)),
                    descriptor,
                }
            } else {
                PropertyValue::Value(new)
