...
impl SubTypingMode {
    ...
    pub(crate) fn one_shallower(self) -> SubTypingMode {
        match self {
            SubTypingMode::Contravariant { depth } => Self::Contravariant {
...
impl<'a> State<'a> {
    ...
    pub fn produce_save_point(&self) -> StateSavePoint {
        ...
        [
            ...
            self.object_constraints
                .as_ref()
                .map_or(0, |c| c.len().try_into().unwrap()),
        ]
    }
    ...
    pub fn reset(&mut self, last: StateSavePoint) {
        ...
        if let Some(ref mut contributions) = self.contributions {
            let _ = contributions
                .staging_covariant
                .drop_range((contributions_covariant as usize)..);
            let _ = contributions
    ...
    match subtype {
        ...
        | Type::AliasTo { to: right, .. } => {
            ...
            return if let (Type::Narrowed { from, .. }, _, true) = (
                subtype,
                &result,
                super::helpers::is_not_of_constant(*right, types),
            ) {
                type_is_subtype_with_generics(
                    (base_type, base_type_arguments),
                ...
                return match arg {
                    ...
                    CovariantContribution::String(string) => {
                        let contributions = state
                            .contributions
                            .as_mut()
                            .map(|n| &mut n.staging_contravariant);
    ...
    match supertype {
        ...
        Type::PartiallyAppliedGenerics(PartiallyAppliedGenerics { on, arguments }) => {
            match *on {
                ...
                TypeId::NO_INFER => {
                    ...
                    let current_contributing = state
                        .contributions
                        .as_ref()
                        .map(|c| c.staging_contravariant.len());
                    let result = type_is_subtype_with_generics(
                        ...
                        (state.contributions.as_mut(), current_contributing)
                    {
                        let _ = contributions
                            .staging_contravariant
                            .drop_range(current_contributing..);
                    }
                    return result;
                }
                TypeId::MULTIPLE_OF => {
                    let argument = arguments
                        .get_structure_restriction(TypeId::NUMBER_GENERIC)
                        .unwrap();
                ...
                TypeId::GREATER_THAN => {
                    ...
                    {
                        let subtype_argument = arguments
                            .get_structure_restriction(TypeId::NUMBER_GENERIC)
                            .unwrap();
                        // Transitivity
            ...
            if let Some(lookup) = types.lookup_generic_map.get(on) {
                ...
                return if let Some(sgs) = get_structure_generics_on(subtype, *on) {
                    ...
                    let prototype = information
                        .get_chain_of_info()
                        .find_map(|info| info.prototypes.get(&ty));
        ...
        Type::Class { .. } => match subtype {
            ...
            Type::Object(..) => {
                ...
                if let Some(prototype) = information
                    .get_chain_of_info()
                    .find_map(|info| info.prototypes.get(&ty))
                {
        ...
        Type::Interface { .. } => {
            ...
            if matches!(
                base_type,
                TypeId::UNDEFINED_TYPE | TypeId::NULL_TYPE | TypeId::NEVER_TYPE
            ) {
                return SubTypeResult::IsNotSubType(NonEqualityReason::Mismatch);
            }
...
fn subtype_floating_properties(
    ...
    subtype_properties(
        (
            base_type,
            reversed_flattened_properties_on_base,
            base_type_arguments,
        ),
    ...
    T: Iterator<Item = &'a (Publicity, PropertyKey<'static>, PropertyValue)> + 'a,
{
        ...
        let key = match key {
            PropertyKey::Type(key_ty) => {
                if let Some(base_type_arguments) = base_type_arguments {
                    let key_ty = base_type_arguments
                        .get_single_argument(*key_ty)
                        .unwrap_or(*key_ty);
    ...
    if property_errors.is_empty() {
        if let Type::Interface {
            ...
            ..
        } = types.get_type_by_id(base_type)
        {
...
pub(crate) fn slice_matches_type(
    ...
    match base_ty {
        ...
        Type::PartiallyAppliedGenerics(PartiallyAppliedGenerics {
            ...
            if matches_constraint {
                let generic_chain_link = Some(GenericChainLink::SpecialGenericChainLink {
                    ...
                    special: SpecialGenericChainLink::CaseTransform {
                        transform: *transform,
                    },
                });
                let inner = arguments
                    .get_structure_restriction(TypeId::STRING_GENERIC)
                    .unwrap();
        ...
        Type::PartiallyAppliedGenerics(PartiallyAppliedGenerics { on, arguments: _ })
            if allow_casts && intrinsics::is_decaf_number_intrinsic(*on) =>
        {
        ...
        Type::Constructor(super::Constructor::KeyOf(on)) => {
            let argument = (
                Publicity::Public,
                &PropertyKey::String(std::borrow::Cow::Borrowed(slice)),
                None,
            );
            ...
            if let Ok(LogicalOrValid::Logical(property)) = property {
                ...
                if let Some(contributions) = contributions {
                    // WIP!!
                    let is_writable = if let Logical::Pure(PropertyValue::Configured {
                        on: _,
                        ref descriptor,
                    }) = property
                    {
