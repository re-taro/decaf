...
pub fn print_type_into_buf<C: InformationChain>(
    ...
    match r#type {
        ...
        Type::Constructor(constructor) => match constructor {
            Constructor::ConditionalResult {
                ...
                if let (TypeId::NEVER_TYPE, Ok(crate::types::TypeExtends { item, extends })) = (
                    *otherwise_result,
                    crate::types::TypeExtends::from_type(*condition, types),
                ) {
                ...
                buf.push_str(if debug || is_standard_generic {
                    " : "
                } else {
                    " | "
                });
            ...
            constructor => {
                if let Constructor::BinaryOperator {
        ...
        | Type::SpecialObject(SpecialObject::Function(func_id, _)) => {
            ...
            if debug {
                let kind = if matches!(r#type, Type::FunctionReference(_)) {
                    "ref"
                } else {
                    ""
                };
                ...
                } = &func.effect
                {
                    write!(
                        buf,
                        "*side effects* {free_variables:?} {closed_over_variables:?}"
                    )
                    .unwrap();
                } else {
