...
impl CallingDiagnostics {
    pub(crate) fn append_to(
        ...
        self.warnings
            .into_iter()
            .for_each(|warning| diagnostics_container.add_warning(warning));
        self.info
            .into_iter()
            .for_each(|crate::diagnostics::InfoDiagnostic(message, position)| {
    ...
    match callable {
        Ok(LogicalOrValid::Logical(callable)) => {
            ...
            let mut check_things = CheckThings {
                debug_types: checking_data.options.debug_types,
            };
        ...
        Err(Invalid(ty)) => {
            checking_data
                ...
                .add_error(TypeCheckError::FunctionCallingError(
...
fn get_logical_callable_from_type(
    ...
    if ty == TypeId::ANY_TYPE {
        ...
        // TODO temp
        return Ok(NeedsCalculation::Infer {
            on: from.unwrap_or(TypeId::ERROR_TYPE),
        }
        .into());
    }
    ...
    match logical {
        Logical::Pure(function) => {
            if let Some(function_type) = types.functions.get(&function.function) {
                ...
                if let FunctionEffect::Constant {
                    identifier: ref const_fn_ident,
                    ..
                } = &function_type.effect
                {
                    let has_dependent_argument = arguments
                        .iter()
                        .any(|arg| types.get_type_by_id(arg.value).is_dependent());
                    ...
                    if call_anyway || !has_dependent_argument {
                        ...
                        match result {
                            ...
                            Ok(ConstantOutput::Diagnostic(diagnostic)) => {
                                diagnostics
                                    .info
                                    .push(InfoDiagnostic(diagnostic, input.call_site));
                                return Ok(CallingOutput {
                                    result: None,
                                    called: None,
                                    result_was_const_computation: !is_independent_function,
                ...
                let structure_generics = if let (false, Some(on)) = (
                    structure_generics.is_some(),
                    function.this_value.get_passed(),
                ) {
        ...
        Logical::Or { .. } => {
            ...
            Err(BadCallOutput {
                returned_type: TypeId::UNIMPLEMENTED_ERROR_TYPE,
            })
    ...
    match types.get_type_by_id(argument.value) {
        ...
        Type::Object(ObjectNature::RealDeal) => {
            top_environment
                ...
                .insert(argument.value, parameter_type);
        ...
        match self.behavior {
            ...
            FunctionBehavior::Method { free_this_id, .. } => {
                ...
                if let SubTypeResult::IsNotSubType(_reason) = type_is_subtype {
                    diagnostics
                        .errors
                        .push(FunctionCallingError::MismatchedThis {
            ...
            if let Some(argument) = argument {
                ...
                if B::CHECK_PARAMETERS {
                    ...
                    if let SubTypeResult::IsNotSubType(_reasons) = result {
                        ...
                        diagnostics
                            .errors
                            .push(FunctionCallingError::InvalidArgumentType {
                                parameter_type:
                    ...
                    type_arguments
                        .arguments
                        .insert(parameter.ty, argument.value);
                }
            } else if parameter.is_optional {
                type_arguments
                    .arguments
                    .insert(parameter.ty, TypeId::UNDEFINED_TYPE);
            } else {
                diagnostics
                    .errors
                    .push(FunctionCallingError::MissingArgument {
                        parameter_position: parameter.position,
                        call_site,
                    });
            }
        ...
        if self.parameters.parameters.len() < arguments.len() {
            if let Some(ref rest_parameter) = self.parameters.rest_parameter {
                ...
                if let Some(mut basis) = basis {
                    ...
                    type_arguments
                        .arguments
                        .insert(rest_parameter.ty, rest_parameter_array_type);
                }
            } else {
