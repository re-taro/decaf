...
impl Default for TypeStore {
    fn default() -> Self {
        ...
        let types = vec![
            ...
            Type::AliasTo {
                to: TypeId::UNDEFINED_TYPE,
                name: "void".into(),
                parameters: None,
            },
            Type::Class {
                name: "Array".to_owned(),
                type_parameters: Some(vec![TypeId::T_TYPE]),
            },
            Type::Class {
            ...
            Type::Interface {
                name: "ImportMeta".to_owned(),
                parameters: None,
                extends: None,
            },
            Type::Constant(Constant::Symbol {
                key: "iterator".to_owned(),
            }),
            Type::Constant(Constant::Symbol {
                key: "asyncIterator".to_owned(),
            }),
            Type::Constant(Constant::Symbol {
                key: "hasInstance".to_owned(),
            }),
            Type::Constant(Constant::Symbol {
                key: "toPrimitive".to_owned(),
            }),
            Type::RootPolyType(PolyNature::StructureGeneric {
                name: "S".into(),
...
impl TypeStore {
    ...
    pub fn new_or_type_from_iterator(&mut self, iter: impl IntoIterator<Item = TypeId>) -> TypeId {
        iter.into_iter()
            .reduce(|acc, n| self.new_or_type(acc, n))
    ...
    pub fn new_conditional_extends_type(
        ...
        let on = self.register_type(Type::Constructor(super::Constructor::TypeExtends(
            TypeExtends {
                item: check_type,
                extends,
            },
    ...
    pub(crate) fn update_alias(&mut self, alias_type: TypeId, to: TypeId) {
        if let Type::AliasTo {
            to: ref mut old, ..
        } = self.types[alias_type.0 as usize]
        {
            *old = to;
        }
    }
