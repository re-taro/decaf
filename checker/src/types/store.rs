...
impl Default for TypeStore {
    fn default() -> Self {
        ...
        let mut types = vec![
            ...
            Type::NamedRooted {
                name: "HTMLElementTagNameMap".to_owned(),
                parameters: None,
            },
            // Internal
            Type::NamedRooted {
        ...
        if let GeneralContext::Syntax(env) = environment.into_general_context() {
            ...
            if environment.context_type.is_dynamic_boundary() {
                ...
                environment
                    .bases
                    .mutable_bases
                    .insert(id, (inference_boundary, TypeId::ANY_TYPE));
                return id;
            }
        }
        TypeId::ANY_TYPE
        ...
        match self.get_type_by_id(on) {
            ...
            Type::Or(left, right) => {
                ...
                let left = self
                    ...
                    .map(Box::new);
                let right = self
                    .get_fact_about_type(ctx, *right, resolver, data)
                    .map(Box::new);
