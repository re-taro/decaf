...
#[must_use]
pub fn is_decaf_number_intrinsic(id: TypeId) -> bool {
    matches!(
        id,
        TypeId::GREATER_THAN | TypeId::LESS_THAN | TypeId::MULTIPLE_OF
    )
}
...
pub fn range_to_type(range: FloatRange, types: &mut TypeStore) -> TypeId {
    ...
    let ceiling = range
        .get_less_than()
        .map(|_number| new_intrinsic(&Intrinsic::LessThan, ceiling_ty, types));
...
pub fn new_intrinsic(intrinsic: &Intrinsic, argument: TypeId, types: &mut TypeStore) -> TypeId {
    ...
    let (on, to_pair) = match intrinsic {
        ...
        Intrinsic::LessThan => {
            ...
            return types.register_type(Type::PartiallyAppliedGenerics(PartiallyAppliedGenerics {
                on: TypeId::LESS_THAN,
                arguments,
            }));
        }
