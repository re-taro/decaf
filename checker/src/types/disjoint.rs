    ...
    if lhs == rhs || lhs == TypeId::ANY_TYPE || rhs == TypeId::ANY_TYPE {
        ...
        ) = (lhs_ty, rhs_ty)
        {
            let rhs_prototype = information
                .get_chain_of_info()
                .find_map(|info| info.prototypes.get(&rhs).copied());
            // {
...
fn number_modulo_disjoint(
    ...
    let argument = types.get_type_by_id(
        arguments
...
fn number_range_disjoint(
    ...
    let this_ty = types.get_type_by_id(
        arguments
            .get_structure_restriction(TypeId::NUMBER_GENERIC)
            .unwrap(),
    );
