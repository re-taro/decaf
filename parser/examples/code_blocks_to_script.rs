...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some((under, extension)) = into_files_directory_and_extension {
        ...
        for (header, code) in blocks {
            ...
            // Fix for FLow
            let code = if replace_satisfies_with_as {
                code.replace(" satisfies ", " as ")
            } else {
                code
            };
        ...
        for (header, mut code) in blocks {
            ...
            if !declare_lets.is_empty() {
                ...
                use decaf_parser::{ast, expressions::operators, functions, Expression, Statement};
                ...
                code = module.to_string(&decaf_parser::ToStringOptions::typescript());
            }

            // If available block add to that, otherwise create a new one
            if let Some((items, block)) = final_blocks
                .iter_mut()
                .find(|(uses, _)| uses.is_disjoint(&names))
            {
...
impl<'a>
    decaf_parser::visiting::Visitor<
        decaf_parser::visiting::ImmutableVariableOrProperty<'a>,
        HashSet<String>,
    > for NameFinder
{
    fn visit(
        &mut self,
        item: &decaf_parser::visiting::ImmutableVariableOrProperty<'a>,
        data: &mut HashSet<String>,
        _chain: &decaf_parser::visiting::Chain,
    ) {
