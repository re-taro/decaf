...
use decaf_parser::{lex_script, LexerOptions};
...
fn lex_and_print_tokens(script: String, print_tokens: bool, top_level_html: bool) {
    ...
    println!("{:?}", decaf_parser::source_map::LineStarts::new(&script));

    let lexer_options: LexerOptions = LexerOptions {
        top_level_html,
        ..Default::default()
    };
    ...
    if print_tokens {
        ...
        while let Some(Token(token, start)) = receiver.next() {
            ...
            let represents = other
                .get((start.0 as usize)..(start.0 as usize + length as usize))
                .unwrap();
