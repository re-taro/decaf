...
impl ASTNode for ExportDeclaration {
    ...
    fn from_reader(
        ...
        let is_default = matches!(
            reader.peek(),
            Some(Token(TSXToken::Keyword(TSXKeyword::Default), _))
        );
        if is_default {
            reader.next();
            let expression = Expression::from_reader(reader, state, settings)?;
            let position = start.union(&expression.get_position());
            Ok(ExportDeclaration::Default {
            ...
            match reader.peek().ok_or_else(parse_lexing_error)? {
                Token(TSXToken::Keyword(TSXKeyword::Class), _) => {
                    ...
                    Ok(Self::Variable {
                        exported: Exportable::Class(class_declaration),
                        position,
                    })
                }
                ...
                Token(TSXToken::Keyword(TSXKeyword::Type), _) => {
                    ...
                    Ok(Self::Variable {
                        exported: Exportable::TypeAlias(type_alias),
                        position,
                    })
                }
                Token(token, _) => {
