...
impl Declaration {
    ...
    pub(crate) fn is_declaration_start(
        ...
        let Some(Token(token, _)) = reader.peek() else {
            return false;
        };
        ...
        return result
            ...
            || {
                let TSXToken::Keyword(token) = *token else {
                    return false;
                };
                let Some(Token(after, _)) = reader.peek_n(1) else {
                    return false;
                };
...
impl ImportLocation {
    pub(crate) fn from_reader(
        ...
        {
            Ok((
                Self::Marker(state.new_partial_point_marker(token.1)),
                source_map::End(token.1 .0),
            ))
        } else {
            Err(ParseError::new(
...
impl ImportExportName {
    pub(crate) fn from_reader(
        ...
        if let Some(Token(TSXToken::Comma, pos)) = reader.peek() {
            ...
            return Ok((
                ImportExportName::Marker(marker),
                pos.union(source_map::End(pos.0)),
            ));
        }
