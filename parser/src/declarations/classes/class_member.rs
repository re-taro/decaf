...
pub enum ClassMember {
    ...
    Function(
        Option<Keyword<tsx_keywords::Static>>,
        ExtractedFunction<ClassFunctionBase>,
    ),
    ...
    fn from_reader(
        ...
        match reader.peek().unwrap() {
            Token(TSXToken::OpenParentheses, _) => {
                ...
                let extracted = state
                    .function_extractor
                    .new_extracted_function(class_method);
...
impl FunctionBased for ClassFunctionBase {
    ...
    fn header_left(header: &Self::Header) -> Option<Cow<Span>> {
        header
            .0
            .as_ref()
            .map(|async_kw| Cow::Borrowed(&async_kw.1))
            .xor(header.1.get_position())
    }
}
