...
impl ASTNode for Expression {
    ...
    fn to_string_from_buffer<T: source_map::ToString>(
        ...
        self.to_string_using_precedence(
            ...
            options,
            local,
            ExpressionToStringArgument {
                on_left: false,
                parent_precedence: u8::MAX,
            },
        );
    }
...
impl Expression {
        ...
        let first_expression = match reader.next().ok_or_else(parse_lexing_error)? {
            ...
            t @ Token(TSXToken::Keyword(TSXKeyword::Import), start) => {
                ...
                if let Some(Token(TSXToken::Dot, _)) = token {
                    ...
                    let options = if reader
                        .conditional_next(|t| matches!(t, TSXToken::Comma))
                        .is_some()
                    {
            ...
            Token(TSXToken::Keyword(TSXKeyword::Yield), s) => {
                ...
                let is_delegated = reader
                    .conditional_next(|t| matches!(t, TSXToken::Multiply))
                    .is_some();

                let operator = if is_delegated {
        ...
        while top.get_precedence() != 2 {
            ...
            match peeked_token {
                ...
                TSXToken::Comment(_) => {
                    ...
                    if reader
                        .peek_n(1)
                        .is_some_and(|t| t.0.is_expression_postfix())
                    {
                    ...
                    if options.type_annotations =>
                {
                    ...
                    let special_operators = match token.0 {
                        ...
                        #[cfg(feature = "extras")]
                        TSXToken::Keyword(TSXKeyword::Is) if options.is_expressions => {
                            SpecialOperators::Is {
                                value: top.into(),
                                type_annotation: Box::new(reference),
                            }
    ...
    pub(crate) fn to_string_using_precedence<T: source_map::ToString>(
        ...
        match self {
            ...
            Self::BinaryAssignmentOperation {
                lhs, operator, rhs, ..
            } => {
                lhs.to_string_from_buffer(buf, options, local);
            ...
            Self::UnaryPrefixAssignmentOperation {
                operand, operator, ..
            } => {
                buf.push_str(operator.to_str());
                operand.to_string_from_buffer(buf, options, local);
            }
            Self::UnaryPostfixAssignmentOperation {
                operand, operator, ..
            } => {
                operand.to_string_from_buffer(buf, options, local);
                buf.push_str(operator.to_str());
            ...
            Self::ArrayLiteral(values, _) => {
                if options.pretty && options.enforce_limit_length_limit() {
                    ...
                    let values_are_all_booleans_or_numbers = values
                        .first()
                        .and_then(ArrayElement::inner_ref)
                        .is_some_and(|e| {
                            matches!(
                                e,
            ...
            Self::ConditionalTernary {
                ...
                truthy_result.to_string_using_precedence(
                    ...
                    local,
                    local2
                        .with_precedence(CONDITIONAL_TERNARY_PRECEDENCE)
                        .on_right(),
                );
                if split_lines {
                    buf.push_new_line();
    ...
    pub(crate) fn to_string_on_left<T: source_map::ToString>(
        ...
        match self {
            ...
            MultipleExpression::Single(single) => {
                let local2 = ExpressionToStringArgument {
                    on_left: true,
                    parent_precedence: u8::MAX,
                };
    ...
    fn from_reader(
        ...
        if let Some(Token(TSXToken::Comma, _)) = reader.peek() {
            ...
            while let Some(Token(TSXToken::Comma, _)) = reader.peek() {
                ...
                let position = top.get_position().union(rhs.get_position());
                top = MultipleExpression::Multiple {
                    lhs: Box::new(top),
                    rhs,
                    position,
                };
...
impl ASTNode for FunctionArgument {
    fn from_reader(
        ...
        match peek {
            ...
            _ => Ok(Self::Standard(Expression::from_reader(
...
impl ASTNode for ArrayElement {
    ...
    fn from_reader(
        ...
        Ok(Self(Some(FunctionArgument::from_reader(
            reader, state, options,
        )?)))
    }
...
pub(crate) fn chain_to_string_from_buffer<T: source_map::ToString>(
    ...
    if split_between_lines && !chain.is_empty() {
        ...
        items
            .next()
            .unwrap()
            .to_string_from_buffer(buf, options, local);
