...
impl ParsingState {
    ...
    fn new_partial_point_marker<T>(&mut self, at: source_map::Start) -> Marker<T> {
        ...
        Marker(
            u8::try_from(id).expect("more than 256 markers"),
            Default::default(),
        )
    }
}
...
impl ExpressionOrStatementPosition for StatementPosition {
    ...
    fn from_reader(
        ...
        VariableIdentifier::from_reader(reader, state, options).map(|identifier| Self {
            identifier,
            is_declare: false,
        })
    }
...
impl ExpressionOrStatementPosition for ExpressionPosition {
    ...
    fn from_reader(
        ...
        | None = reader.peek()
        {
            ...
            Ok(Self(Some(VariableIdentifier::from_reader(
...
fn receiver_to_tokens(
    ...
    std::iter::from_fn(move || {
        ...
        if last == start {
            ...
            let token = input
                .get((last as usize)..(start as usize))
                .unwrap_or("?")
                .to_owned();
            last = span.end;
            Some((token, false))
