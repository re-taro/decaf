...
pub fn lex_script(
    ...
    if script.len() > u32::MAX as usize {
        return Err((
            LexingErrors::CannotLoadLargeFile(script.len()),
            source_map::Nullable::NULL,
        ));
    }

    let mut state: LexingState = if options.top_level_html {
        ...
        match state {
                    ...
                    GetNextResult::Result {
                        ...
                        match result {
                            ...
                            TSXToken::MultiLineComment(_) => {
                                state = LexingState::MultiLineComment {
                                    last_char_was_star: false,
                                };
                                continue;
                            }
                            _ => {}
            ...
            LexingState::JSXLiteral {
                ...
                match jsx_state {
                    ...
                    JSXLexingState::AttributeKey => match chr {
                        ...
                        chr => {
                            let character_allowed = chr.is_alphanumeric()
                                || chr == '-'
                                || (options.allow_unsupported_characters_in_jsx_attribute_keys
                                    && matches!(
                                        chr,
                                        '@' | ':' | '.' | '[' | ']' | '+' | '$' | '*' | '%'
                                    ));
                    ...
                    JSXLexingState::LiteralContent {
                        ref mut last_char_was_open_chevron,
                    } => match chr {
                        '<' => {
