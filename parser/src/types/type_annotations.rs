    ...
    fn from_reader(
        ...
        if let Some(Token(TSXToken::Colon, _)) = reader.peek_n(1) {
            ...
            Ok(AnnotationWithBinder::Annotated {
                position: pos.union(ty.get_position()),
                name,
                ty,
            })
        } else {
        ...
        match self {
            ...
            Self::Conditional {
                ...
                let split_lines = crate::are_nodes_over_length(
                    [condition, resolve_true, resolve_false]
                        .iter()
                        .map(AsRef::as_ref),
                    options,
                    local,
    ...
    pub(crate) fn from_reader_with_config(
        ...
        let mut reference = match reader.next().ok_or_else(parse_lexing_error)? {
            ...
            t @ Token(TSXToken::Keyword(TSXKeyword::Symbol), _) => {
                ...
                #[cfg(feature = "extras")]
                let name = reader
                    .conditional_next(|t| matches!(t, TSXToken::StringLiteral(..)))
                    .map(|t| {
        ...
        if let Some(Token(TSXToken::Dot, _)) = reader.peek() {
            ...
            while reader
                .conditional_next(|tok| *tok == TSXToken::Dot)
                .is_some()
            {
        ...
        while reader
            .conditional_next(|tok| *tok == TSXToken::OpenBracket)
            .is_some()
        {
...
impl TypeAnnotationFunctionParameters {
    pub(crate) fn from_reader_sub_open_parenthesis(
        ...
        while !matches!(reader.peek(), Some(Token(TSXToken::CloseParentheses, _))) {
            ...
            if reader
                .conditional_next(|tok| matches!(tok, TSXToken::Comma))
                .is_none()
            {
                break;
            }
    ...
    fn from_reader(
        ...
        let (kind, position) = if let Some(spread) = is_spread {
            (
                ...
                spread.1.union(annotation_with_binder.get_position()),
            )
            ...
            reader.conditional_next(|token| matches!(token, TSXToken::QuestionMark))
        {
            (
                ...
                annotation_with_binder
                    .get_position()
                    .union(trailing_question.get_end()),
            )
        } else {
            (
                TupleElementKind::Standard,
                annotation_with_binder.get_position(),
            )
        };
...
mod tests {
    ...
    fn name() {
        ...
        assert_matches_ast!(
            "string",
            TypeAnnotation::CommonName(CommonTypes::String, span!(0, 6))
        );
    }
