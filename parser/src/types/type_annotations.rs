        ...
        match self {
            ...
            Self::Intersection(items) | Self::Union(items) => Cow::Owned(
                items
                    ...
                    .unwrap()
                    .get_position()
                    .union(&items.last().unwrap().get_position()),
            ),
        }
    }
}
...
impl TypeAnnotation {
    ...
    pub(crate) fn from_reader_with_config(
        ...
        let mut reference = match reader.next().ok_or_else(parse_lexing_error)? {
            ...
            | Token(TSXToken::DoubleQuotedStringLiteral(content), pos) => {
        ...
        if let Some(Token(TSXToken::Dot, _)) = reader.peek() {
            reader.next();
            let (name, start) = if let Self::Name(name, start) = reference {
                (name, start)
            } else {
                panic!()
            };
            ...
            Some(Token(TSXToken::BitwiseOr, _)) => {
                ...
                while let Some(Token(TSXToken::BitwiseOr, _)) = reader.peek() {
                    ...
                    union_members.push(Self::from_reader_with_config(
                        reader, state, settings, true, false,
                    )?);
                }
