...
impl ASTNode for StatementOrDeclaration {
        ...
        if Declaration::is_declaration_start(reader, options) {
                ...
                reader.peek()
            {
                if reader.peek_n(1).map_or(false, |t| !t.0.is_symbol()) {
                    return Ok(StatementOrDeclaration::Declaration(
                        Declaration::from_reader(reader, state, options)?,
                    ));
                }
            }
    ...
    fn to_string_from_buffer<T: source_map::ToString>(
        ...
        match self {
            ...
            StatementOrDeclaration::Imported {
                moved,
                from,
                originally: _,
            } => {
    ...
    fn visit<TData>(
        ...
        if options.visit_nested_blocks || chain.is_empty() {
            ...
            if options.reverse_statements {
                items
                    .rev()
                    .for_each(|item| item.visit(visitors, data, options, chain));
            } else {
