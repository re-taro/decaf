...
impl<T: FunctionBased> FunctionBase<T> {
    pub(crate) fn from_reader_with_header_and_name(
        ...
        let position = header_left
            .unwrap_or_else(|| parameters.position.get_start())
            .union(end_pos);
...
impl ASTNode for FunctionHeader {
    ...
    fn from_reader(
        ...
        let async_start = state
            .optionally_expect_keyword(reader, TSXKeyword::Async)
            .map(|kw| kw.get_start());
...
pub enum MethodHeader {
    ...
    Regular {
        is_async: bool,
        generator: Option<GeneratorSpecifier>,
    },
}
    ...
    pub(crate) fn from_reader(
        ...
        match reader.peek() {
            ...
            Some(Token(TSXToken::Keyword(TSXKeyword::Generator), _)) => {
                Some(GeneratorSpecifier::Keyword)
            }
            _ => None,
        }
    }
...
pub(crate) fn get_method_name<T: PropertyKeyKind + 'static>(
    ...
    let (function_header, key) = if is_named_get_set_or_async {
        ...
        (
            ...
            WithComment::None(PropertyKey::Identifier(
                name.to_owned(),
                position,
                new_public,
            )),
...
impl ASTNode for FunctionBody {
    fn get_position(&self) -> Span {
        self.0
            .as_ref()
            .map_or(source_map::Nullable::NULL, |Block(_, pos)| *pos)
    }
