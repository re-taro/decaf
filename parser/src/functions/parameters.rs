    ...
    fn to_string_from_buffer<T: source_map::ToString>(
        ...
        if options.enforce_limit_length_limit() && local.should_try_pretty_print {
            ...
            for parameter in parameters {
                parameter
                    .name
                    .to_string_from_buffer(&mut buf, options, local);
        ...
        if let Some(rest_parameter) = rest_parameter {
            ...
            rest_parameter
                .name
                .to_string_from_buffer(buf, options, inner_local);
        ...
        loop {
                ...
                reader.conditional_next(|tok| matches!(tok, TSXToken::Spread))
            {
                ...
                let position = spread_pos.union(
                    type_annotation
                        .as_ref()
                        .map_or(name_position, ASTNode::get_position),
                );
                ...
                let constraint = TypeAnnotation::from_reader(reader, state, options)?;
                let position = start.union(constraint.get_position());
                super_type = Some(SuperParameter {
                    constraint,
                    position,
                });
            } else {
