        ...
        match self {
            ...
            VariableIdentifier::Marker(_, _) => {
                assert!(
                    ...
                    "variable marker attempted to convert to string"
                );
            }
        }
    }
        ...
        match reader.peek().ok_or_else(parse_lexing_error)?.0 {
            TSXToken::OpenBrace => {
                ...
                Ok(Self::Object {
                    members,
                    spread,
                    position: start_pos.union(last_pos),
                })
            }
            TSXToken::OpenBracket => {
                ...
                Ok(Self::Array {
                    members,
                    spread,
                    position: start_pos.union(end),
                })
            }
            _ => Ok(Self::Name(VariableIdentifier::from_reader(
...
impl<T: DestructuringFieldInto> ASTNode for ObjectDestructuringField<T> {
    fn from_reader(
        ...
        if reader
            .peek()
            .is_some_and(|Token(t, _)| is_destructuring_into_marker(t, options))
        {
            reader.next();
