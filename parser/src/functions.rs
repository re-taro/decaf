...
impl<T: FunctionBased> FunctionId<T> {
    pub fn new() -> Self {
        Self(
            FUNCTION_ID_COUNTER.fetch_add(1, Ordering::SeqCst),
            Default::default(),
        )
    }
    ...
    T::Body: Visitable,
{
    fn visit<TData>(
        ...
        self.parameters
            .visit(visitors, data, settings, functions, chain);
    ...
    fn visit_mut<TData>(
        ...
        self.parameters
            .visit_mut(visitors, data, settings, functions, chain);
        if settings.visit_function_bodies {
            self.body
                .visit_mut(visitors, data, settings, functions, chain);
        }
