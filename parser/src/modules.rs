...
impl ASTNode for Module {
    ...
    fn from_reader(
        ...
        let span = Span {
            start,
            source: (),
            end: start + state.length_of_source,
        };
...
impl Module {
    pub fn visit<TData>(
...

        {
            visitors.visit_block(
                &crate::block::BlockLike { items: &self.items },
                data,
                &chain,
            );
        }
        ...
        if options.reverse_statements {
            ...
            iter.rev()
                .for_each(|item| item.visit(visitors, data, options, &mut chain));
        }
    }
    ...
    pub fn visit_mut<TData>(
        ...
        if options.reverse_statements {
            ...
            iter_mut
                .rev()
                .for_each(|item| item.visit_mut(visitors, data, options, &mut chain));
        }
    }
}
