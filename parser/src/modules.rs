    ...
    fn from_reader(
        ...
        parse_statements_and_declarations(reader, state, settings).map(|statements| {
            ...
            let source = statements
                .last()
                .map(|stmt| stmt.get_position().source)
    ...
    pub fn visit<TData>(
...

        {
            visitors.visit_block(
                &mut crate::block::BlockLike { items: &self.items },
                data,
                &chain,
            );
        }
...
impl ASTNode for TypeDefinitionModule {
    ...
    fn from_reader(
        ...
        loop {
            declarations.push(TypeDefinitionModuleDeclaration::from_reader(
    ...
    fn from_reader(
        ...
        match reader.peek().ok_or_else(parse_lexing_error)? {
            ...
            Token(TSXToken::Keyword(TSXKeyword::Interface), _) => {
                ...
                Ok(TypeDefinitionModuleDeclaration::Interface(Decorated {
                    decorators,
                    on,
                }))
            }
            Token(TSXToken::Keyword(TSXKeyword::Type), _) => {
                Ok(TypeDefinitionModuleDeclaration::LocalTypeAlias(
                    TypeAlias::from_reader(reader, state, settings)?,
                ))
            }
...
pub(crate) fn parse_declare_item(
    ...
    match reader.peek() {
        ...
        Some(Token(TSXToken::Keyword(TSXKeyword::Type), _)) => {
            Ok(TypeDefinitionModuleDeclaration::TypeAlias(
                TypeAlias::from_reader(reader, state, settings)?,
            ))
        }
        Some(Token(TSXToken::Keyword(TSXKeyword::Namespace), _)) => {
            Ok(TypeDefinitionModuleDeclaration::Namespace(
                Namespace::from_reader(reader, state, settings)?,
            ))
        }
        _ => {
            let Token(token, position) = reader.next().unwrap();
