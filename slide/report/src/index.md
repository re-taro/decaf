...
- `any` 型により、型チェックを回避可能
  - **JavaScript からの移行を容易にする柔軟性**を提供
- **漸進的型付け（Gradual Typing）を謳っている**

---

## 背景 | Gradual Typing とは

Gradual Typing は**静的型付けと動的型付けを融合させる手法**
...
## 背景 | TypeScript と Gradual Typing

TypeScript の売りは JavaScript からの移行が容易なこと

--> 型アノテーションを省略しても JavaScript と同じように動作する

Gradual Typing の基準に照らし合わせると、省略した型検査はランタイムで検出すべき

--> TypeScript はランタイムで型エラーとして検出できない

---

## 問題提起

**TypeScript の柔軟性を保つための選択肢は**
**Gradual Typing だけなのだろうか**
...
- **ランタイムエラーを静的解析時に検出する**静的型検査機を実装

--> 漸進的型付けでは無く、**静的型検査による健全性**を重視
...
| **n**     | 1           | 10     | 20     | 40           |
| --------- | ----------- | ------ | ------ | ------------ |
| **decaf** | &nbsp;19.47 | 162.54 | 484.79 | 1610.00      |
